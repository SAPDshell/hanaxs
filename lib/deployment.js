...
module.exports = function(dir, config, callback) {
	...
	exec("regi.exe create workspace --force", options, function(err, stdout, stderr) {
		exec("regi.exe rebase package " + config.root, options, function(err, stdout, stderr) {
			exec("regi.exe resolve package " + config.root + " --with=local", options, function(err, stdout, stderr) {
