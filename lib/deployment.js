...
module.exports = function(dir, config, callback) {
	...
	regi("create workspace --force", function(err, stdout, stderr) {
		regi("track " + s(config.pattern).template({root: config.root, user: global.username}).s, function(err, stdout, stderr) {
			regi("checkout", function(err, stdout, stderr) {
				regi("rebase", function(err, stdout, stderr) {
					regi("resolve package " + s(config.pattern).template({root: config.root, user: global.username}).s + " --with=local", function(err, stdout, stderr) {
						rimraf(dir + "/" + s(config.pattern).template({root: config.root, user: global.username}).s, function() {
